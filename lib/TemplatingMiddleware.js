...
var TemplatingMiddleware = new Class({
    ...
    , init: function initialize(environments){
        ...
        if (argv.has('dev')) this.env = 'dev';
        else if (argv.has('testing')) this.env = 'testing';
        else if (argv.has('live')) this.env = 'live';
    ...
    , request: function (request, response, next) {
        ...
        response.render = function (status, language, headers, data, callback) {
            ...
            var context = {content: data, template: template, language:language, host: request.hostname, env: this.env};
            // check if the service has overwritten the content type and chose a new renderer
            if(headers['content-type'] && headers['content-type'] !== renderer.type){
                var ctype = headers['content-type'],
                    splitTypes = ctype.split('/');

                var newRenderer = this.createRenderer(request, splitTypes[0], splitTypes[1]);
